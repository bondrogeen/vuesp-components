...
const { items, itemKey = 'id' } = defineProps<Props>();

const emit = defineEmits<{
  (e: 'update:items', items: any[]): void;
  (e: 'order-change', payload: { fromIndex: number; toIndex: number; movedItem: any; newOrder: any[] }): void;
}>();
...
const handleContainerDrop = (event: DragEvent) => {
  ...
  if (draggingId.value && dragOverIndex.value !== -1) {
    const fromIndex = items.findIndex((item: any) => item[itemKey] === draggingId.value);
